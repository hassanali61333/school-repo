...
const user = useSelector((state) => state.users.loginuser);
  ...
  const subjectsByGroup = {
    ...
    'Commerce': ['Accounting', 'Economics', 'Business Studies', 'Mathematics', 'Computer Science'],
  ...
  const validateForm = () => {
    ...
    if (rollNo.length < 1 || rollNo.length > 10) newErrors.rollNo = 'Roll number must be 1-10 digits';
    if (!studentEmail) newErrors.studentEmail = 'Student email is required';
    if (!studentEmail.includes('@')) newErrors.studentEmail = 'Invalid email format';
    if (!studentPassword || studentPassword.length < 6) newErrors.studentPassword = 'Password must be at least 6 characters';
    ...
    if (parentPhone.length < 10 || parentPhone.length > 14) newErrors.parentPhone = 'Phone must be 10-14 digits';
    if (!parentEmail) newErrors.parentEmail = 'Parent email is required';
    if (!parentEmail.includes('@')) newErrors.parentEmail = 'Invalid email format';
    if (!parentPassword || parentPassword.length < 6) newErrors.parentPassword = 'Password must be at least 6 characters';
    ...
    if (selectedSubjects.length === 0) newErrors.selectedSubjects = 'Select at least one subject';
    if (!monthlyFee || parseFloat(monthlyFee) <= 0) newErrors.monthlyFee = 'Monthly fee must be greater than 0';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Success case
      const successMsg = response.data.message || 'Student successfully admitted!';
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <p className="text-gray-600 mt-1">Fill in the details to admit a new student</p>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-md">
                  ...
                  <p><strong>Class:</strong> {selectedClass} - Section {selectedSection}</p>
                  {group && <p><strong>Group:</strong> {group}</p>}
                  <p><strong>Subjects:</strong> {selectedSubjects.join(', ') || 'To be assigned'}</p>
                </div>

                <div className="bg-gray-50 p-4 rounded-md">
                  <h3 className="font-semibold text-lg mb-2">Fee Details</h3>
                  <p><strong>Monthly Fee:</strong> PKR {monthlyFee}</p>
                  <p><strong>Due Day:</strong> {dueDay} of each month</p>
                </div>
              </div>
